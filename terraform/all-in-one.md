...
| <a name="input_use_target_as_builder"></a> [use\_target\_as\_builder](#input_use_target_as_builder)                   | Use target host as remote nix builder. When enabled with kexec phase, boots kexec first so target has nix available, then builds using target as remote builder. Speeds up builds significantly when deploying from macOS to Linux.                       | `bool`                                                                    | `false`                                                                      |    no    |
