...
main() {
  ...
  if [[ ${machineSubstituters} == "y" && -n ${flake} ]]; then
    substituters=$(nix --extra-experimental-features 'nix-command flakes' eval --json "${flake}"#"${flakeAttr}".nix.settings.substituters | jq -r '. | join(" ")')
    trustedPublicKeys=$(nix --extra-experimental-features 'nix-command flakes' eval --json "${flake}"#"${flakeAttr}".nix.settings.trusted-public-keys | jq -r '. | join(" ")')

    runSsh "mkdir -p ~/.config/nix && touch ~/.config/nix/nix.conf && echo extra-substituters = ${substituters} >> ~/.config/nix/nix.conf"
    runSsh "echo extra-trusted-public-keys = ${trustedPublicKeys} >> ~/.config/nix/nix.conf"
  fi
